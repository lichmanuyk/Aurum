/** Pure donut-sector geometry — no React import, deliberately: this is the
 * one piece of sectorConnector.ts's own logic that e2e/tests/
 * donut-chart-interaction.spec.ts also imports directly (to check the
 * *real* rendered route's segments against the *real* rendered donut, not
 * a synthetic re-derivation of the same numbers). The e2e project has its
 * own, separate node_modules (see e2e/package.json) with no "react" in it
 * — CI only ever runs `npm ci` there, never in frontend/ (see
 * .github/workflows: browser-tests only installs e2e/'s deps before
 * `bash e2e/run.sh`) — so anything importable from a browser test has to
 * resolve with zero runtime dependencies, react included. sectorConnector.ts
 * re-exports all of this unchanged for its own (React-owning) consumers. */
...
export const CHART_MARGIN_PX = 5;
 ...
 * at. Exported for direct unit testing — see sectorGeometry.test.ts. */
 ...
 * see sectorGeometry.test.ts. */
export function outerArcPoint(midAngleDeg: number, boxWidth: number, boxHeight: number): Point {
...
export const ROUTE_HORIZONTAL_RUN_PX = 16;
