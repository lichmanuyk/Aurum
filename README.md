...
`origin/personal`. После установки отдельная локальная копия ветки проверяет
изменения каждые 5 минут, пока Mac включён; рабочий checkout может оставаться
на любой ветке. Перед пересборкой существующего проекта `aurum-personal`
