...
На этом Mac обновление можно автоматизировать после появления коммита в
`origin/personal`. После установки отдельная копия ветки в
`~/Library/Application Support/Aurum/personal-deploy` проверяет изменения
каждые 5 минут, пока Mac включён; рабочий checkout может оставаться на любой
ветке. Конфигурация хранится в `~/Library/Application Support/Aurum/personal.env`,
а `.env.personal` в репозитории ссылается на неё. Перед пересборкой существующего проекта `aurum-personal`
создаётся проверенная резервная копия в iCloud Drive, затем проверяется
доступность приложения на порту 3003. При ошибке запуска скрипт пытается
вернуть прежний код, сообщает об ошибке и сохраняет копию данных для
восстановления. Неудачный коммит не запускается повторно каждые 5 минут;
при несовместимой миграции БД может потребоваться ручное восстановление
из копии. Том БД и Tailscale Serve не меняются.

Установка на этом Mac после слияния изменений автоматизации в `personal`:

```bash
python3 scripts/install_personal_auto_deploy.py
```

Журнал: `~/Library/Logs/Aurum/auto-deploy.out.log` и `auto-deploy.err.log`.
Проверка службы: `launchctl list | grep com.aurum.personal-auto-deploy`.
При выключенном Mac обновление выполнится после следующего входа в систему.
Автообновление срабатывает только при линейном обновлении `personal` и
отказывается заменять локально изменённую копию для деплоя.
...
В целях можно выбрать счёт и зарезервировать часть его остатка. Деньги не
списываются и капитал не удваивается; в списке счетов отдельно видны полный,
зарезервированный и доступный остатки. Отрицательный взнос снимает резерв с
той же цели и счёта.
...
Проверка восстановления последней копии: `python3 scripts/check_backup_restore.py`.
Она проверяет свежесть и SHA-256, импортирует копию в одноразовую базу и
сравнивает все разделы после экспорта. Рабочую базу на порту 3003 не меняет;
временные контейнеры и том удаляются после проверки.
