...
omitting year/month entirely now means "all time" (year=None too means
"every month" is meaningless), a year with no month means "every month of
that year", and year+month keeps working exactly as before. Plain
month-scoping arithmetic (income/expense/net, category breakdown) is
already covered by test_dashboard.py — this file covers what's new: the
three period shapes themselves, that "ends today" clips a stray
future-dated transaction out of a period that's supposed to end today, and
that transfers/adjustments/asset trades/splits behave the same way across
all three modes as they already do for a single month.
"""
...
async def test_no_params_at_all_means_all_time_from_the_earliest_activity_through_today(
...
async def test_month_without_year_is_rejected(client: AsyncClient):
    resp = await client.get("/dashboard/summary", params={"month": 5})
    assert resp.status_code == 422


async def test_year_without_month_covers_every_month_of_that_past_year(client: AsyncClient, account_id, categories):
    salary = categories["Salary"]["id"]
    ...
    resp = await client.get("/dashboard/summary", params={"year": 2020})
    body = resp.json()
...
async def test_all_time_and_the_current_years_all_months_clip_to_today_not_a_stray_future_date(
    ...
    all_time = (await client.get("/dashboard/summary")).json()
    assert money(all_time["real_income"]) == Decimal("100.00")

    whole_year = (await client.get("/dashboard/summary", params={"year": today.year})).json()
...
async def test_transfers_adjustments_asset_trades_and_splits_behave_the_same_in_every_period_mode(
    ...
    body = (await client.get("/dashboard/summary")).json()  # all time
...
async def test_missing_historical_rate_is_an_explicit_error_in_all_time_mode_too(client: AsyncClient, categories):
    ...
    response = await client.get("/dashboard/summary")  # no year/month at all -> all time
